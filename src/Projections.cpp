...
std::string ReturnFormattedHistogramNameFromProjection(ProjectionVariable Proj) {
  ...
  for (size_t iKinematicCut = 0; iKinematicCut < Proj.KinematicCuts.size(); iKinematicCut++) {
    ...
    ReturnStr += Form("(%4.2f < %s < %4.2f)", Proj.KinematicCuts[iKinematicCut].Range[0],
                      Proj.KinematicCuts[iKinematicCut].Name.c_str(),
                      Proj.KinematicCuts[iKinematicCut].Range[1]);
  }
...
int main(int argc, char *argv[]) {
  ...
  for (auto &ProjectionConfig : fitMan->raw()["Projections"]) {
    ...
    for (auto &CategoryCutConfig : ProjectionConfig["CategoryCuts"]) {
      ...
      CategoryCut Cut = CategoryCut{CategoryCutName, CategoryCutVarString, CategoryCutBreakdown,
                                    CategoryCutColours, CategoryCutNames};
  ...
  for (size_t iProj = 0; iProj < Projections.size(); iProj++) {
    ...
    if (Projections[iProj].KinematicCuts.size() > 0) {
      ...
      for (size_t iCut = 0; iCut < Projections[iProj].KinematicCuts.size(); iCut++) {
        MACH3LOG_INFO( "\t\t\tCut {:<2} - Name : {:<20} , Lower Bound : {:<10} , Upper Bound : {:<10}", iCut,
            Projections[iProj].KinematicCuts[iCut].Name,
            Projections[iProj].KinematicCuts[iCut].Range[0],
            Projections[iProj].KinematicCuts[iCut].Range[1]);
      }
    }
  ...
  for (size_t iProj = 0; iProj < Projections.size(); iProj++) {
    ...
    for (auto Sample : DUNEPdfs) {
      ...
      for (size_t iCut = 0; iCut < Projections[iProj].KinematicCuts.size(); iCut++) {
        ...
        Selection[0] = Sample->ReturnKinematicParameterFromString(
            Projections[iProj].KinematicCuts[iCut].VarString);
      ...
      for (size_t iCat = 0; iCat < Projections[iProj].CategoryCuts.size(); iCat++) {
        MACH3LOG_INFO("\t\tCategory: {:<10} - Name : {:<20}", iCat,
                      Projections[iProj].CategoryCuts[iCat].Name);
        ...
        for (size_t iBreak = 0; iBreak < Projections[iProj].CategoryCuts[iCat].Breakdown.size(); iBreak++) {
          ...
          for (size_t iGroup = 0; iGroup < Projections[iProj].CategoryCuts[iCat].Breakdown[iBreak].size(); iGroup++) {
            ...
            Hist = Sample->get1DVarHist(ProjectionVar_Str, SelectionVector_IncCategory, WeightStyle,
                                        &Axis);
            std::cout << "Colour: " << Projections[iProj].CategoryCuts[iCat].Colours[iBreak]
                      << std::endl;
