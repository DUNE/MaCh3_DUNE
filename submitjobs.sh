...
elif [[ $HOSTNAME == cedar* || $HOSTNAME == login* || $HOSTNAME == gra-login* || $HOSTNAME == beluga[0-9]* ]]; then
  IsCedar=true
...
for ((i = 0 ; i < $NCHAINS ; i++)); do
  ...
  #sed -i "s|^\( *OutputFile.*:\).*|\1 \"${OutputDir}/${JobName}.root\"|g" "$cfgFileName"
  sed -i "s|^\([[:space:]]*OutputFile:\).*|\1 \"${OutputDir}/${JobName}.root\"|g" "$cfgFileName"

  grep "OutputFile" "$cfgFileName"

  AdaptiveJobName=${FitName}_adaptive_chain_${i}_job_${JOB}

  #sed -i "s|^\([[:space:]]*OutputFileName:\).*|\1 \"${OutputDir}/${AdaptiveJobName}.root\"|g" "$cfgFileName"
  sed -i "/AdaptionOptions:/,/Covariance:/ s|^\([[:space:]]*OutputFileName:\).*|\1 \"${OutputDir}/${AdaptiveJobName}.root\"|g" "$cfgFileName"
...
#done
  ...
  echo "---- Adaptive output path ----"
  grep -n "OutputFileName" "$cfgFileName"
  echo "------------------------------"
