...
for (int i = 0; i < nXsecBranches && i < prefitValues.size(); ++i) {
    ...
    validEntries.reserve(nEntries);

    std::cout << "[INFO] Building list of post-burn-in entries..." << std::endl;
    ...
    //     }
    // }
    // Disconnect all branches, then only activate "step"
   // Binary search for first entry where step >= burn_in
...
if (firstValid < 0) {
    std::cerr << "[ERROR] No valid MCMC entries after burn-in!" << std::endl;
    return 1;
}

Long64_t nValid = nEntries - firstValid;
std::cout << "[INFO] First post-burn-in entry: " << firstValid 
          << ", valid entries: " << nValid << std::endl;

// Re-enable all branches and re-bind
...
// Then in your posterior draw loop, just offset by firstValid
for (int p = 0; p < no_times_sampling_posterior; ++p) {
    Long64_t chosen = firstValid + rnd->Integer(nValid);
    mcmc->GetEntry(chosen);
    // ... rest of loop unchanged
}

    std::cout << "[INFO] Found "
            << validEntries.size()
            << " valid posterior entries after burn-in"
            << std::endl;

    if (validEntries.empty()) {
        std::cerr << "[ERROR] No valid MCMC entries after burn-in!"
                << std::endl;
        return 1;
    }
    ...
    Long64_t chosen =
        validEntries[rnd->Integer(validEntries.size())];
    ...
    for (auto& pdf : DUNEPdfs) {
        ...
        MakeSpectaVariations(pdf, "TrueNeutrinoEnergy",
                             fOut, ND_or_FD, pdfTitle, p);

        MakeSpectaVariations(pdf, "RecoNeutrinoEnergy",
                             fOut, ND_or_FD, pdfTitle, p);

        MakeSpectaVariations(pdf, "Enubias",
                             fOut, ND_or_FD, pdfTitle, p);
    }
