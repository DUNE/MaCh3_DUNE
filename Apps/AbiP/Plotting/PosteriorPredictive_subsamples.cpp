...
// sample_idx selects the subsample within pdf (default 0, i.e. old behavior)
// ---------------------------------------------------------------------------
                          ...
                          int                sample_idx = 0)
{
    ...
    if (!h) {
        std::cerr << "[WARN] Could not get histogram for " << var
                  << " from " << pdfTitle << " (subsample " << sample_idx << ")" << std::endl;
        return;
    }
    ...
    if (p < -1) {
        ...
        histName = Form("%s_%s_subsample%d_%s_AsimovFakeData",
                    ND_or_FD.c_str(), cleanTitle.c_str(), sample_idx, var.c_str());
    } else if (p < 0) {
        baseDir  = "Asimov/" + ND_or_FD + "/" + var;
        histName = Form("%s_%s_subsample%d_%s_Asimov",
                        ND_or_FD.c_str(), cleanTitle.c_str(), sample_idx, var.c_str());
    } else {
        baseDir  = ND_or_FD + "/" + var;
        histName = Form("%s_%s_subsample%d_%s_posterior_toy_%03d",
                        ND_or_FD.c_str(), cleanTitle.c_str(), sample_idx, var.c_str(), p);
    }
...
int main(int argc, char* argv[])
{
    ...
    // Optional: number of subsamples per SampleHandlerFD, in the same order as
    // DUNEPdfs is built by MakeMaCh3DuneInstance. If not present in the config,
    // defaults to 1 per pdf (i.e. identical behavior to before this change).
    // Add to your cfg e.g.:
    //   Predictive:
    //     NSubSamplesPerPdf: [3, 2, 1]
    auto NSubSamplesPerPdf =
        GetFromManager<std::vector<int>>(fitMan->raw()["Predictive"]["NSubSamplesPerPdf"], {});
     std::cout << "[DEBUG] got here, size=" << NSubSamplesPerPdf.size() << std::endl;
    // -----------------------------------------------------------------------
    // Output file
    // -----------------------------------------------------------------------
    ...
    // Resolve subsample counts: use config if provided and correctly sized,
    // otherwise default to 1 per pdf and warn.
    std::vector<int> nSubSamples(DUNEPdfs.size(), 1);
    if (!NSubSamplesPerPdf.empty()) {
        if (NSubSamplesPerPdf.size() == DUNEPdfs.size()) {
            nSubSamples = NSubSamplesPerPdf;
        } else {
            std::cerr << "[WARN] Predictive:NSubSamplesPerPdf has "
                      << NSubSamplesPerPdf.size() << " entries but there are "
                      << DUNEPdfs.size() << " DUNEPdfs — ignoring and defaulting to 1 each."
                      << std::endl;
        }
    } else {
        std::cout << "[INFO] Predictive:NSubSamplesPerPdf not set in config — "
                     "defaulting to 1 subsample per pdf." << std::endl;
    }
    ...
    for (size_t iPdf = 0; iPdf < DUNEPdfs.size(); ++iPdf) {
        auto& pdf = DUNEPdfs[iPdf];
        pdf->Reweight();

        for (int s = 0; s < nSubSamples[iPdf]; ++s) {
            std::string pdfTitle = pdf->GetSampleTitle(s);
            std::cout << "[DEBUG] Raw sample title: \"" << pdfTitle
                      << "\" (subsample " << s << ")" << std::endl;
    ...
    for (int p = 0; p < no_times_sampling_posterior; ++p) {
        ...
        for (size_t iPdf = 0; iPdf < DUNEPdfs.size(); ++iPdf) {
            ...
            for (int s = 0; s < nSubSamples[iPdf]; ++s) {
                ...
                MakeSpectaVariations(pdf, "TrueNeutrinoEnergy", fOut, ND_or_FD, pdfTitle, p, s);
                MakeSpectaVariations(pdf, "RecoNeutrinoEnergy",  fOut, ND_or_FD, pdfTitle, p, s);
                MakeSpectaVariations(pdf, "Enubias",             fOut, ND_or_FD, pdfTitle, p, s);
            }
        }
        fOut->cd();
    }
