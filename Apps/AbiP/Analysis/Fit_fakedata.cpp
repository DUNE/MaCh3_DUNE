...
int main(int argc, char * argv[]) {
  ...
  TFile* FakeData = TFile::Open("/scratch/abipeake/MaCh3_DUNE_makingsplines/MaCh3_DUNE/fakedatasystset_eventrate.root", "READ");
  //TFile* FakeData = TFile::Open("/scratch/abipeake/MaCh3_DUNE_makingsplines/MaCh3_DUNE/offaxis_fakedataeventrate.root", "READ");

    if (!FakeData || FakeData->IsZombie()) {
    MACH3LOG_ERROR("Could not open data file");
 ...
 for (unsigned sample_i = 0 ; sample_i < DUNEPdfs.size() ; ++sample_i) {
  ...
  //std::string sampleName = DUNEPdfs[sample_i]->GetTitle();
// e.g. "OffAxisND_0m"
 std::string sampleName = DUNEPdfs[sample_i]->GetTitle(); // e.g. "OffAxisND_0m"
sample_names.push_back(sampleName);

TString NameTString(sampleName.c_str());



  std::string histName =
  "hRecoNeutrinoEnergy_ELepRec" + sampleName;
// e.g. "hRecoNeutrinoEnergy_ELepRecOffAxisND_0m"

TH1* dataHist = dynamic_cast<TH1*>(FakeData->Get(histName.c_str()));
if (!dataHist) {
    MACH3LOG_ERROR("Missing data histogram '{}'", histName);
    throw MaCh3Exception(__FILE__, __LINE__);
}

TH1* dataClone = static_cast<TH1*>(dataHist->Clone(NameTString + "_data"));
dataClone->SetDirectory(nullptr);

// if (DUNEPdfs[sample_i]->GetNDim() == 1) {
//     DUNEPdfs[sample_i]->AddData(static_cast<TH1D*>(dataClone));
// } else if (DUNEPdfs[sample_i]->GetNDim() == 2) {
//     DUNEPdfs[sample_i]->AddData(static_cast<TH2D*>(dataClone));
// }
  ...
  if (DUNEPdfs[sample_i]->GetNDim() == 1) {
    ...
    MACH3LOG_ERROR("Unsupported number of dimensions > 2");
    throw MaCh3Exception(__FILE__, __LINE__);
  }
}
...
//     MACH3LOG_INFO("{} : {}",name.c_str(),dataHist->Integral());
//     MACH3LOG_INFO("--------------");
//   }
