...
void Save1DSlicesWithEventRateAndCorr(
    TH2D* h_event1, TH2D* h_event_err1,
    TH2D* h_post1,  TH2D* h_post_err1,
    const TMatrixD& corr1,

    TH2D* h_event2, TH2D* h_event_err2,
    TH2D* h_post2,  TH2D* h_post_err2,
    const TMatrixD& corr2,
...
{
    int nSlices = sliceAlongQ3 ? h_post1->GetNbinsY() : h_post1->GetNbinsX();
    int nBinsX  = h_post1->GetNbinsX();
    int nBinsY  = h_post1->GetNbinsY();

    for (int i = 1; i <= nSlices; ++i) {
        c->Clear();

        // --- Slice boundaries ---
        double lowEdge  = sliceAlongQ3 ? h_post1->GetYaxis()->GetBinLowEdge(i)
                                       : h_post1->GetXaxis()->GetBinLowEdge(i);
        double highEdge = sliceAlongQ3 ? h_post1->GetYaxis()->GetBinUpEdge(i)
                                       : h_post1->GetXaxis()->GetBinUpEdge(i);
        ...
        // --- Event rate slices ---
        TH1D* h_event_slice1 = sliceAlongQ3
            ? h_event1->ProjectionX(Form("h_event1_slice_%d", i), i, i)
            : h_event1->ProjectionY(Form("h_event1_slice_%d", i), i, i);

        TH1D* h_event_slice2 = sliceAlongQ3
            ? h_event2->ProjectionX(Form("h_event2_slice_%d", i), i, i)
            : h_event2->ProjectionY(Form("h_event2_slice_%d", i), i, i);

        h_event_slice1->SetLineColor(kBlue+1);
        h_event_slice1->SetMarkerStyle(20);

        h_event_slice2->SetLineColor(kRed+1);
        h_event_slice2->SetMarkerStyle(24);

        h_event_slice1->SetXTitle(formatAxisTitle(xvar1).c_str());
        h_event_slice1->SetYTitle("Event rate / GeV");
        h_event_slice1->SetTitle(sliceLabel.c_str());

        // --- Posterior slices ---
        auto makePostSlice = [&](TH2D* h_post, TH2D* h_err) {
            TH1D* h_post_slice;
            if (sliceAlongQ3) {
                h_post_slice = new TH1D("", "", nBinsX, h_post->GetXaxis()->GetXbins()->GetArray());
                for (int x = 1; x <= nBinsX; ++x) {
                    double val = h_post->GetBinContent(x, i);
                    double err = h_err->GetBinContent(x, i);
                    h_post_slice->SetBinContent(x, val);
                    h_post_slice->SetBinError(x, err);
                }
            } else {
                h_post_slice = new TH1D("", "", nBinsY, h_post->GetYaxis()->GetXbins()->GetArray());
                for (int y = 1; y <= nBinsY; ++y) {
                    double val = h_post->GetBinContent(i, y);
                    double err = h_err->GetBinContent(i, y);
                    h_post_slice->SetBinContent(y, val);
        ...
        TH1D* h_post_slice1 = makePostSlice(h_post1, h_post_err1);
        TH1D* h_post_slice2 = makePostSlice(h_post2, h_post_err2);

        h_post_slice1->SetLineColor(kBlack);
        h_post_slice1->SetMarkerStyle(21);
        h_post_slice1->GetYaxis()->SetRangeUser(0.9,1.1);
        h_post_slice1->SetXTitle(formatAxisTitle(xvar1).c_str());
        h_post_slice1->SetYTitle("Posterior / GeV");

        h_post_slice2->SetLineColor(kRed+2);
        h_post_slice2->SetMarkerStyle(25);

        // --- Correlation slices ---
        std::vector<int> sliceInds = GetSliceIndices(binDefs, i, sliceAlongQ3, h_post1);
        TMatrixD sliceCorr1 = GetSliceCorrelation(corr1, sliceInds);
        TMatrixD sliceCorr2 = GetSliceCorrelation(corr2, sliceInds);

        TH2D* h_corr_slice1 = PlotSliceCorrelation(sliceCorr1, param_names, sliceInds, i);
        TH2D* h_corr_slice2 = PlotSliceCorrelation(sliceCorr2, param_names, sliceInds, i);

        // --- Canvas layout ---
        c->Divide(2,1);

        // Left pad: stacked slices (event + posterior, both fits)
        TPad* padLeft = (TPad*)c->cd(1);
        padLeft->Divide(1,2,0,0);
        padLeft->cd(1);
        h_event_slice1->Draw("E1");
        h_event_slice2->Draw("E1 SAME");

        TLegend* leg1 = new TLegend(0.7,0.7,0.9,0.9);
        leg1->AddEntry(h_event_slice1,"Fit 1","P");
        leg1->AddEntry(h_event_slice2,"Fit 2","P");
        leg1->Draw();

        padLeft->cd(2);
        h_post_slice1->Draw("E1");
        h_post_slice2->Draw("E1 SAME");

        TLegend* leg2 = new TLegend(0.7,0.7,0.9,0.9);
        leg2->AddEntry(h_post_slice1,"Fit 1","P");
        leg2->AddEntry(h_post_slice2,"Fit 2","P");
        leg2->Draw();

        // Right pad: correlation matrices (side by side)
        c->cd(2);
        TPad* padRight = new TPad("padRight","",0,0,1,1);
        padRight->Divide(2,1);
        padRight->Draw();
        padRight->cd(1); h_corr_slice1->Draw("COLZ");
        padRight->cd(2); h_corr_slice2->Draw("COLZ");

        c->Update();
        c->Print(pdfOut.c_str());

        // cleanup
        delete h_event_slice1;
        delete h_event_slice2;
        delete h_post_slice1;
        delete h_post_slice2;
        delete h_corr_slice1;
        delete h_corr_slice2;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 7) {
        std::cerr << "Usage: " << argv[0] << " <mcmc_output1.root> <config.yaml> <corr_matrix1.root> <mcmc_output2.root> <config2.yaml>  <corr_matrix2.root>" << std::endl;
        return 1;
    }
    ...
    std::string mcmc_file2 = argv[4];
    std::string config_file2 = argv[5];
    std::string corr_file2 = argv[6];

    // -------------------------
    // Open original MCMC file in READ mode
    // -------------------------
    ...
    if (!post) {
        std::cerr << "Error: No non-empty 'posteriors' tree found in "
                << "Onaxis_EventRates_Elep_erec_afteradaptive_chain_1_job_0.root" 
                << std::endl;
        return 1;
    }

    // -------------------------
    // Open second MCMC file in READ mode
    // -------------------------
    TFile* f_post2 = TFile::Open(mcmc_file2.c_str(), "READ");
    if (!f_post2 || f_post2->IsZombie()) {
        std::cerr << "Error: cannot open MCMC file " << mcmc_file2 << std::endl;
        return 1;
    }
    TTree* post2 = getLatestNonEmpty(f_post2);
    ...
    std::cout << "Using " << post->GetName() 
            << " with " << post->GetEntries() << " entries." 
            << std::endl;

    std::cout << "Using " << post2->GetName() 
            << " with " << post2->GetEntries() << " entries." 
            << std::endl;



    // -------------------------
    // Open correlation matrix file in READ mode
    // -------------------------
    ...
    // -------------------------
    // Open second correlation matrix file in READ mode
    // -------------------------
    TFile* fCorr2 = TFile::Open(corr_file2.c_str(), "READ");
    if (!fCorr2 || fCorr2->IsZombie()) {
        std::cerr << "Error: cannot open correlation file " << corr_file2 << std::endl;
        return 1;
    }
    TMatrixDSym* corrMatrix2 = (TMatrixDSym*) fCorr2->Get("Correlation");
    if (!corrMatrix2) {
        std::cerr << "Error: Correlation not found in " << corr_file2 << std::endl;
        return 1;
    }
    ...
    // Load YAML config
    // -------------------------
    YAML::Node config2 = YAML::LoadFile(config_file2);
    manager* FitManager2 = new manager(config_file2);
    if (!FitManager2) {
        std::cerr << "Error: Failed to create FitManager from YAML config: " << config_file2 << std::endl;
        return 1;
    }
    ...
    //For set 2
    covarianceXsec* xsec2 = nullptr;
    std::vector<samplePDFFDBase*> DUNEPdfs2;
    MakeMaCh3DuneInstance(FitManager2, DUNEPdfs2, xsec2, osc);
    std::unique_ptr<mcmc> MaCh3Fitter2 = std::make_unique<mcmc>(FitManager2);
    ...
    // ======================================================
//  SECOND SET OF 2D HISTOGRAMS (Fit #2 / Post #2)
// ======================================================

// -------------------------
// Build combined 2D histogram (second fit)
// -------------------------
TH2D* h_q0q3_combined_2 = nullptr;
for (auto& pdf : DUNEPdfs2) {   // <-- use second PDF set
    auto* dunePdf = dynamic_cast<samplePDFDUNEBeamFD*>(pdf);
    if (!dunePdf) continue;
    ...
    TH2* h = dunePdf->get2DVarHist(xsec_var1, xsec_var2, SelectionVector, 0, axisX, axisY);
    if (!h) {
        std::cerr << "Warning: get2DVarHist returned null (second fit).\n";
        continue;
    }
    if (!h_q0q3_combined_2) {
        h_q0q3_combined_2 = (TH2D*)h->Clone("h_q0q3_combined_2");
        h_q0q3_combined_2->SetDirectory(nullptr);
    } else {
        h_q0q3_combined_2->Add(h);
    }
}

if (!h_q0q3_combined_2) {
    std::cerr << "Error: No 2D histograms were produced (second fit).\n";
    return 1;
}

h_q0q3_combined_2->GetXaxis()->SetTitle(xsec_var1.c_str());
h_q0q3_combined_2->GetYaxis()->SetTitle(xsec_var2.c_str());
h_q0q3_combined_2->Write("xsec_eventrate_histo_2");

// --- Event rate histogram with adaptive label colors (second fit) ---
TH2D* h_eventrate_labels_2 = (TH2D*)h_q0q3_combined_2->Clone("h_eventrate_labels_2");
h_eventrate_labels_2->SetTitle("Event Rate with Bin Labels (Fit #2);q_{0} [GeV];q_{3} [GeV]");

gStyle->SetOptStat(0);
gStyle->SetPalette(kPastel);

h_eventrate_labels_2->Draw("COLZ");

int nBinsX_2 = h_eventrate_labels_2->GetNbinsX();
int nBinsY_2 = h_eventrate_labels_2->GetNbinsY();
double textSize_2 = 0.5 * std::min(1.0/nBinsX_2, 1.0/nBinsY_2);
if (textSize_2 > 0.05) textSize_2 = 0.05;

TLatex latex_2;
latex_2.SetTextAlign(22);
latex_2.SetTextSize(textSize_2);

for (const auto& bin : binDefs) {
    double q0_center = 0.5 * (bin.q0_min + bin.q0_max);
    double q3_center = 0.5 * (bin.q3_min + bin.q3_max);

    int binX = h_eventrate_labels_2->GetXaxis()->FindBin(q0_center);
    int binY = h_eventrate_labels_2->GetYaxis()->FindBin(q3_center);
    double binContent = h_eventrate_labels_2->GetBinContent(binX, binY);

    int color = kBlack;
    double maxContent = h_eventrate_labels_2->GetMaximum();
    if (binContent / maxContent > 0.5) color = kWhite;

    latex_2.SetTextColor(color);
    latex_2.DrawLatex(q0_center, q3_center, std::to_string(bin.index).c_str());
}

// -------------------------
// Posterior mean/stddev histograms (second fit)
// -------------------------
TH2D* h_mean_2 = new TH2D("h_param_mean_2", "Posterior Mean (Fit #2);q0 [GeV];q3 [GeV]",
                          q0_edges.size()-1, &q0_edges[0],
                          q3_edges.size()-1, &q3_edges[0]);
TH2D* h_stddev_2 = new TH2D("h_param_stddev_2", "Posterior StdDev (Fit #2);q0 [GeV];q3 [GeV]",
                            q0_edges.size()-1, &q0_edges[0],
                            q3_edges.size()-1, &q3_edges[0]);
    ...
    // ======================================================
//  Posterior predictive distributions (Fit #2)
// ======================================================
...
////////////////////////////////////////////////////////////////////////////
    ...
    // -------------------------
    // Add plots to multipage PDF (Fit #2)
    // -------------------------
    c_master->Clear();
    h_eventrate_labels_2->Draw("COLZ TEXT");
    c_master->Print(outputPdfFile.c_str());

    c_master->Clear();
    h_q0q3_combined_2->Draw("COLZ");
    c_master->Print(outputPdfFile.c_str());
    ...
    c_master->Clear();
    h_stddev_2->Draw("COLZ");
    c_master->Print(outputPdfFile.c_str());

    c_master->Clear();
    //h_eventrate_mean_2->Draw("COLZ");
    c_master->Print(outputPdfFile.c_str());

    c_master->Clear();
    //h_eventrate_stddev_2->Draw("COLZ");
    c_master->Print(outputPdfFile.c_str());

    Save1DSlicesWithEventRateAndCorr(
        h_eventrate_labels, h_eventrate_stddev,
        h_mean, h_stddev,
        *corrMatrix,                // <- THIS should be the TMatrixD
        h_eventrate_labels, h_eventrate_stddev,
        h_mean_2, h_stddev_2,
        *corrMatrix2,               // <- THIS should be the TMatrixD
        binDefs,
        paramNames,
        c_master,
        outputPdfFile,
