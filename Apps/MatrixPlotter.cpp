//MaCh3 Includes
#include "/scratch/abipeake/MaCh3_DUNE_merged/MaCh3_DUNE/build/_deps/mach3-src/plotting/plottingUtils/plottingUtils.h"
#include "/scratch/abipeake/MaCh3_DUNE_merged/MaCh3_DUNE/build/_deps/mach3-src/plotting/plottingUtils/plottingManager.h"
...
void PlotMatrix(std::string Config, std::string File)
{
  ...
  gStyle->SetPaintTextFormat(" "); // render blanks if text mode is on
  ...
  MatrixFull->SetMarkerSize(0); // ⬅ hides bin text
  ...
  for(size_t it = 0; it < Title.size(); it++)
  {
    ...
    Hist->SetMarkerSize(0); // ⬅ hides bin text for sub-matrix
    Hist->Draw("COLZ");
    MatrixPlot->Print("MatrixPlot.pdf");
    delete Hist;
  }
...
int main(int argc, char *argv[]) 
{
  ...
  std::cout << "This is the MatrixPlotter I am compiling ... " << std::endl;
