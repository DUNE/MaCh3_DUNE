...
void extract_2D_bins_from_yaml(const std::string& yaml_file, std::vector<BinDef>& binDefs, std::vector<double>& q0_edges, std::vector<double>& q3_edges) {
...
void FixLowStatParams(covarianceXsec* xsec, TH1D* h_1D, double total_events, double frac_threshold, std::vector<std::string>& fixed_names_out, const std::vector<BinDef>& binDefs) {
    ...
    for (const auto& bin : binDefs) {
        ...
        int bin_q0 = h_1D->GetXaxis()->FindBin(q0);

        if (bin_q0 <= 0 || bin_q0 > h_1D->GetNbinsX()){
            std::cerr << " Bin index out of bounds: q0_bin=" << bin_q0 << std::endl;
            continue;
        }

        double val = h_1D->GetBinContent(bin_q0);
        double sqrt_bin = sqrt(val);
        std::cout << "Param " << bin.index << " at (q0=" << q0 << ") → Bin content: " << val << "sqrt bin content = " << sqrt_bin << " error = " << (sqrt_bin/val)*100 << std::endl;
        if (val < threshold) {
            //std::cout << " → Frozen";
            xsec->setSingleParameter(bin.index, 1.0);////set to 1.0 instead
...
int main(int argc, char* argv[]) {
  ...
  std::ifstream test("/scratch/abipeake/MaCh3_DUNE_merged/MaCh3_DUNE/configs/CovObjs/TrueNeutrinoEnergy_ERecProxy_minus_Enu_0.0_5.0GeV_fullgrid_smallerbins.yaml");
    if (!test.is_open()) {
    std::cerr << "File not found: /scratch/abipeake//MaCh3_DUNE/configs/CovObjs/TrueNeutrinoEnergy_ERecProxy_minus_Enu_0.0_5.0GeV_fullgrid_smallerbins.yaml" << std::endl;
    return 1;
  }
  extract_2D_bins_from_yaml(xsec_param_yaml, binDefs, q0_edges, q3_edges);
  ...
  */
  ...
  for (auto& pdf : DUNEPdfs) {
    ...
    TH1* h = dunePdf->get1DVarHist("RecoNeutrinoEnergy");
    if (!h_q0q3_combined)
        h_q0q3_combined = (TH1D*)h->Clone("h_q0q3_combined");
    else
        h_q0q3_combined->Add(h);
}
