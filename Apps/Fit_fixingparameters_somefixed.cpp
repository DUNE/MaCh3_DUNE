...
int main(int argc, char* argv[]) {
  ...
  TH2D* h_flat = (TH2D*)DUNEPdfs[0]->get2DHist()->Clone("h_flat");
  for (size_t i = 1; i < DUNEPdfs.size(); ++i){
    h_flat->Add(DUNEPdfs[i]->get2DHist());
  }
  double h_flat_integral = h_flat->Integral();
  std::cout << " h_flat_integral" << std::endl;
...
std::cout << "✅ Extracted " << q0_edges.size() << " q0 edges and " << q3_edges.size() << " q3 edges" << std::endl;
  ...
  TH2D* h_q0q3 = new TH2D("h_q0q3", "E_{#nu} vs E^{proxy}_{#nu} - E^_{#nu} ; q_{3} [GeV]; q_{0} [GeV]",
  ...
  /*
  for (int bin_q0 = 1; bin_q0 <= nbins_q0; ++bin_q0) {
    for (int bin_q3 = 1; bin_q3 <= nbins_q3; ++bin_q3) {
      
      int global_bin = h_flat->GetBin(bin_q3, bin_q0); // Correct 2D bin index
      double content = h_flat->GetBinContent(global_bin);
      h_q0q3->SetBinContent(bin_q3, bin_q0, content);
    }
  }*/

  /*
 int nBinsQ0 = h_q0q3->GetNbinsX();
int nBinsQ3 = h_q0q3->GetNbinsY();

int bin_flat = 1; // ROOT histograms start at bin 1

for (int bin_q0 = 1; bin_q0 <= nBinsQ0; ++bin_q0) {
    for (int bin_q3 = bin_q0; bin_q3 <= nBinsQ3; ++bin_q3) {
        double content = h_flat->GetBinContent(bin_flat);
        h_q0q3->SetBinContent(bin_q0, bin_q3, content);
        ++bin_flat;
    }
}*/
  ...
  std::cout << "Number of bins in binDefs: " << binDefs.size() << std::endl;
  ...
  for (const auto& bin : binDefs) {
    ...
    std::cout << "q0: " << q0 << " bin_q0: " << bin_q0 << " / " << h_q0q3->GetNbinsY()
          << " | q3: " << q3 << " bin_q3: " << bin_q3 << " / " << h_q0q3->GetNbinsX() << std::endl;
  ...
  c->Print((pdfOutName + "[").c_str());
  //c->Print("param_summary_q0q3_someparamsfrozen.pdf[");
  h_event_rate->Draw("COLZ");     c->Print("param_summary_q0q3_someparamsfrozen.pdf");
  h_param_init->Draw("COLZ");     c->Print("param_summary_q0q3_someparamsfrozen.pdf");
  h_param_frozen->Draw("COLZ");   c->Print("param_summary_q0q3_someparamsfrozen.pdf");
  h_param_mean->Draw("COLZ");     c->Print("param_summary_q0q3_someparamsfrozen.pdf");
  h_param_stddev->Draw("COLZ");   c->Print("param_summary_q0q3_someparamsfrozen.pdf");
  c->Print((pdfOutName + "]").c_str());
